...
const OperationSurveyServiceGetSurveyTemplate = "/api.survey.v1.SurveyService/GetSurveyTemplate"
...
type SurveyServiceHTTPServer interface {
	...
	// GetSurveyTemplate ===== TEMPLATES (Optional Admin/Setup) =====
	GetSurveyTemplate(context.Context, *GetSurveyTemplateRequest) (*SurveyTemplate, error)
...
func RegisterSurveyServiceHTTPServer(s *http.Server, srv SurveyServiceHTTPServer) {
	...
	r.GET("/v1/survey-templates/{id}", _SurveyService_GetSurveyTemplate0_HTTP_Handler(srv))
...
func _SurveyService_GetSurveyTemplate0_HTTP_Handler(srv SurveyServiceHTTPServer) func(ctx http.Context) error {
	return func(ctx http.Context) error {
		var in GetSurveyTemplateRequest
		if err := ctx.BindQuery(&in); err != nil {
			return err
		}
		if err := ctx.BindVars(&in); err != nil {
		...
		http.SetOperation(ctx, OperationSurveyServiceGetSurveyTemplate)
		h := ctx.Middleware(func(ctx context.Context, req interface{}) (interface{}, error) {
			return srv.GetSurveyTemplate(ctx, req.(*GetSurveyTemplateRequest))
		})
		out, err := h(ctx, &in)
		if err != nil {
			return err
...
type SurveyServiceHTTPClient interface {
	...
	// GetSurveyTemplate ===== TEMPLATES (Optional Admin/Setup) =====
	GetSurveyTemplate(ctx context.Context, req *GetSurveyTemplateRequest, opts ...http.CallOption) (rsp *SurveyTemplate, err error)
	// ListSurveyTemplates ===== TEMPLATES (Optional Admin/Setup) =====
...
// GetSurveyTemplate ===== TEMPLATES (Optional Admin/Setup) =====
func (c *SurveyServiceHTTPClientImpl) GetSurveyTemplate(ctx context.Context, in *GetSurveyTemplateRequest, opts ...http.CallOption) (*SurveyTemplate, error) {
	var out SurveyTemplate
	pattern := "/v1/survey-templates/{id}"
	path := binding.EncodeURL(pattern, in, true)
	opts = append(opts, http.Operation(OperationSurveyServiceGetSurveyTemplate))
	opts = append(opts, http.PathTemplate(pattern))
	err := c.cc.Invoke(ctx, "GET", path, nil, &out, opts...)
	if err != nil {
		return nil, err
